...
struct Weather
{
   ...
   let tempMin: Double
    ...
    init(weatherData: [String: AnyObject])
    {
        ...
        dateString = format.string(from: dateAndTime)
        ...
        sunRiseTime = format.string(from: sunrise)
        sunSetTime = format.string(from: sunset)
    }
}
...
class WeatherGetter
{
    ...
    let apiKEY = "ce983596c0938bc21785ec95b6b251e7"
    ...
    {
        let getURL = URL(string: "\(apiURL)?APPID=\(apiKEY)&q=\(city)")!
        getWeather(getURL)
    }
    
    func getWeatherByCoordinates(latitude: Double, longitude: Double)
    {
        let getURL = URL(string: "\(apiURL)?APPID=\(apiKEY)&lat=\(latitude)&lon=\(longitude)")!
        getWeather(getURL)
    }
